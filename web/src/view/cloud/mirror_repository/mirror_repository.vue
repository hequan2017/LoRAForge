  ...
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker
       ...
       <el-form-item label="来源" prop="source">
            <el-input v-model="searchInfo.source" placeholder="搜索来源" />
       </el-form-item>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column align="left" label="名字" prop="name" width="200" />
    ...
    <template #default="scope">{{ formatBoolean(scope.row.isListed) }}</template>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateMirrorRepositoryFunc(scope.row)">编辑</el-button>
...
import {
  ...
  deleteMirrorRepository,
  deleteMirrorRepositoryByIds,
  updateMirrorRepository,
  findMirrorRepository,
  getMirrorRepositoryList
...
const getTableData = async() => {
  const table = await getMirrorRepositoryList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteMirrorRepositoryFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteMirrorRepositoryByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateMirrorRepositoryFunc = async(row) => {
    const res = await findMirrorRepository({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteMirrorRepositoryFunc = async (row) => {
    const res = await deleteMirrorRepository({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
