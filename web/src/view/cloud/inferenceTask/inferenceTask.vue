...
<template>
  ...
  <el-input v-model="searchInfo.modelPath" placeholder="搜索条件" />
</el-form-item>
            
            <el-form-item label="状态" prop="status">
    <el-tree-select v-model="searchInfo.status" placeholder="请选择状态" :data="inference_statusOptions" style="width:100%" filterable :clearable="false" check-strictly ></el-tree-select>
</el-form-item>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column align="left" label="模型路径" prop="modelPath" width="120" />

            <el-table-column align="left" label="采样步数" prop="steps" width="120" />

            <el-table-column align="left" label="引导系数" prop="cfgScale" width="120" />

            <el-table-column align="left" label="随机种子" prop="seed" width="120" />

            <el-table-column align="left" label="宽度" prop="width" width="120" />

            <el-table-column align="left" label="高度" prop="height" width="120" />

            <el-table-column align="left" label="采样器" prop="sampler" width="120" />

            <el-table-column align="left" label="状态" prop="status" width="120">
    <template #default="scope">
    {{ filterDict(scope.row.status,inference_statusOptions) }}
    </template>
</el-table-column>
            <el-table-column label="生成结果" prop="resultImage" width="200">
    <template #default="scope">
      <el-image preview-teleported style="width: 100px; height: 100px" :src="getUrl(scope.row.resultImage)" fit="cover"/>
    </template>
</el-table-column>
            <el-table-column align="left" label="耗时(秒)" prop="generationTime" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button v-auth="btnAuth.edit" type="primary" link icon="edit" class="table-button" @click="updateInferenceTaskFunc(scope.row)">编辑</el-button>
...
const getTableData = async() => {
  const table = await getInferenceTaskList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteInferenceTaskByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateInferenceTaskFunc = async(row) => {
    const res = await findInferenceTask({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteInferenceTaskFunc = async (row) => {
    const res = await deleteInferenceTask({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
